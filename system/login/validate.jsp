...
String phone=request.getParameter("phone");
 ...
 %>
	...
	<body style="text-align:left;background:#f5f5f5;">
	<div class="regcon">
        <a class="m-fnlogo fn-fl" href="">
          <img src="<%=contextPath %>/libs/images/demo/tmlogo.png" width=110 height=110,alt="筷赚" style="margin-top: -21px;margin-left: 38px;" />
        </a><span class="m-fntit">用户验证</span>
        <div class="ui_tab">
            <ul class="ui_tab_nav regnav">
               
                <li class="no fn-fr loginbtn"></li>
            </ul> 
                    ...
                    <div class="fnlogin fnlogin3 clearfix">
                      <form id="myFormId" method="post" action="">
                        ...
                        <div class="line">
                            ...
                            <div class="liner">
                                <input type="text" class="inp fmobile J_cellphone validate[required,custom[mobilephone]]" readonly="readonly" name="userPhone"  value="<%=phone%>" placeholder="请输入手机号" /><p class="fn-fl errorbox v-txt" id="mobileBox">手机号码输入不正确</p>
                            </div>
                        </div>
                         <div class="line">
                        ...
                        <div class="line">
                            <label class="linel"><span class="dt"><span class="star">*</span>设置密码：</span></label>
                            <div class="liner">
                        ...
                        <div class="line liney clearfix">
                            ...
                            <div class="liner">
                                <a class="regbtn J_btn_agree" id="regbtn" href="javascript:;" onClick="register('#myFormId')">验证</a>
                            </div>
                    </div>
 </form> 
                    </div>
...
function pwdblur(pwd){
	var $this=$(pwd);
	 var thatInput = $this.parents('.line').prev().find('.J_password');
     var pw2 = $this.val();
     var pw2Format = validateRules.isPwdRepeat(pw2,thatInput.val());
     if(!pw2Format){
    	 alert("两次密码输入不一致");
     } 
...
function daojishi(seconds){
 ...
 function sendMsg(){
    ...
    if(isPhone){       
        	var formUrl="<%=contextPath%>/register/getMobilePhoneCode.do";
            $.ajax({
        		type:"post", //请求方式
        		url:formUrl, //发送请求地址
...
function register(containerId){
 ...
 if(valid == true){
	...
	var formUrl="<%=contextPath%>/user/validate.do";
	$.ajax({
		type:"post", //请求方式
		url:formUrl, //发送请求地址
		data:{"userid":<%=userid%>,"userPassword":userPassword,"userPhone":<%=phone%>},
		//请求成功后的回调函数有两个参数
		success:function(data){
			
			if(data == 1){
				alert("验证成功！");
				window.location="<%=contextPath%>/system/login/grant.jsp?shopid=<%=shopid%>&userid=<%=userid%>" ;
			}else if(data == 0){
				alert("验证失败！");
			}else if(data == 2){
				alert("用户不存在");
